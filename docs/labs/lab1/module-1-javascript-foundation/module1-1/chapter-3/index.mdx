...
const formatPrice = (price) => `${price} บาท`;

const prices = [55, 80, 120];
const vatRate = 0.07;

const withVat = prices.map((p) => calcVat(p, vatRate));
const premium = withVat.filter((p) => p > 100);

console.log(withVat); // [58.85, 85.6, 128.4]
console.log(premium); // [128.4]
...
console.log(box.normal()); // 10 (this ชี้ box)
console.log(box.arrow());  // undefined (this มาจากบริบทนอกสุด)
```
