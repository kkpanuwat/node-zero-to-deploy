...
console.log("c (var) ยังมองเห็น:", c);

// 2) var hoist
console.log("x ก่อนประกาศ:", x); // undefined
var x = 10;

// 3) let TDZ
try {
  console.log(y); // จะเข้า catch
} catch (err) {
  console.log("y error (TDZ):", err.message);
}
let y = 5;
...
// 5) var ใน loop → ใช้ let/const
for (let i = 0; i < 3; i++) {
  setTimeout(() => {
    console.log("ใช้ let จับค่า i ถูกต้อง:", i);
  }, 0);
}
```
