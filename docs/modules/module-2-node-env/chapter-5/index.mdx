...
### คำสั่งสำคัญที่ใช้
- `http.createServer((req, res) => { ... })`: สร้างเซิร์ฟเวอร์และจัดการทุก request ผ่าน callback
- `req.method`, `req.url`: ใช้ตรวจว่าเป็น GET/POST และเส้นทางอะไร
- `res.writeHead(statusCode, headers)`: ตั้งสถานะและ header (เช่น `Content-Type`)
- `res.end(body)`: จบการตอบกลับและส่งข้อมูลไปยัง client
- `JSON.stringify(data)`: แปลง object/array เป็นสตริง JSON เพื่อส่งกลับ
- `process.uptime()`: เวลาเซิร์ฟเวอร์ทำงาน (วินาที) ใช้ใน `/status`
