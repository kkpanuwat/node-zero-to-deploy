...
- **(Optional) Quickstart** — [Node + Express (80m)](./3-node-express-quickstart.md): เวอร์ชันย่อสำหรับเริ่มต้นรัน Node/Express อย่างไว (เน้น CommonJS)
- **Part 3** — [Hands-on Lab: Books CRUD API (Express + JSON)](../day-2-nodejs-express/lab.md): **(เริ่มลงมือทำที่นี่)** ทำ CRUD + persistence แบบอ่าน/เขียนไฟล์ JSON
